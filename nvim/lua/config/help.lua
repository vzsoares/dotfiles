...
local SECTIONS = {
    ...
    },
    {
        "LSP & Code",
        {
            ...
            { "<F2>",       "Rename symbol" },
            { "<F4>",       "Spell suggestions (on a misspelled word) / Code action" },
    ...
    },
    {
        "Debug — DAP (Go / delve)",
        {
            ...
            { "<F11>",       "Step into" },
            { "<F12>",       "Step out" },
            { "<F9>",        "Step back (rr replay sessions only)" },
            { "<leader>dc",  "Reverse continue (rr replay sessions only)" },
            { "<leader>dg",  "Jump to cursor line — no rewind (Python only)" },
            { "<leader>dF",  "Restart current frame (JS/TS only)" },
            { "<F6>",        "Session menu (terminate / restart / UI / REPL)" },
            { "<leader>b",   "Toggle breakpoint (persisted per project)" },
            { "<leader>B",   "Conditional breakpoint" },
            { "<leader>de",  "Eval value under cursor / selection" },
            { "<leader>dt",  "Debug Go test nearest cursor" },
            { "<leader>du",  "Toggle debug UI" },
            { "<leader>dR",  "Restart session" },
            { "<leader>dr",  "Open REPL" },
            { "",            "Go katas need a go.mod (module) to launch" },
            { "",            "No breakpoint set? one is added on main()" },
            { ":GoRecord",   "Record this package with rr, then F5 > Replay" },
            { "",            "Stepping back needs a recording; rr replay only" },
        },
    },
    {
